...
class RouteMiddleware extends Middleware
{
    ...
    public function __invoke(object $next): object
    {
        if ($this->compare()) {
            ...
            throw new Exceptions(
                "The {$this->routeClass->http('request')->route} route was not found.",
                404
            );
        }
    }
    ...
    public function compare(): bool
    {
        ...
        if (array_key_exists($uri, $routes) !== false) {
            ...
            $this->routeClass->http('request')->controller
                = $datas['controller'];
    ...
    private function routesOfAController(string $uri): RouteMiddleware
    {
        ...
        if (!empty($this->request->base)) {
            if (substr($uri, 0, stripos($uri, DS)) == substr(
                    $this->request->base,
                    1
                )
            ) {
        ...
        foreach ($routes as $key => $values) {
            if (substr($uri, 0, stripos($uri, DS)) != $key) {
                ...
                $count = count($keys);
                for ($a = 0; $a < $count; $a++) {
                    if (stripos($keys[$a], explode(DS, substr($uri, $len))[0])
                        !== false
                    ) {
                        $found = true;
                        break;
        ...
        if ($count == 2) {
            if (count($route) > 0) {
                foreach (array_keys($route) as $key) {
                    $this->routeClass->http('request')->params[substr(
                        $route[$key],
                        1,
                        -1
                    )]
    ...
    private function exchange($data): RouteMiddleware
    {
        ...
        if (isset($data['controller']) && isset($data['action'])) {
            ...
            throw new Exceptions(
                "This route contains their respective control and action."
            );
        }
        return $this;
    }
