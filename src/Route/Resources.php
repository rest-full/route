...
class Resources
{
    ...
    private function add(string $method, string $route, string $handler, $callback = null): Resources
    {
        ...
        if (isset($callback)) {
            $this->request->callback[$route] = [
                $callback,
                (new Instances())->anonymousFunction($callback)
            ];
            return $this;
        }
    ...
    private function createRouteFilter(string $path, string $handler, bool $activeName): Resources
    {
        ...
        for ($a = 1; $a >= 0; $a--) {
            ...
            $name = $a == 1 ? ' with pagination'
                : ' with ordenation and pagination';
            if ($a == 0) {
                $newPath .= DS . '{' . $this->parameters['filter'][($a + 1)]
                    . '}';
            }
            foreach (
            (stripos($path, 'list') ? $this->methods[0] : $this->methods[9]) as
                                                                             $method
            ) {
                $this->add($method, $newPath, $handler);
            }
            if ($activeName) {
                $this->name[$newPath] = (stripos($path, 'list')
                        ? $this->names[0] : $this->names[9]) . $name;
            }
        }
        return $this;
    }
